...
class image_clustering:
	...
	def __init__(self, folder_path="data", n_clusters=10, max_examples=None, use_imagenets=False, use_pca=False):
		...
		for i in range(self.n_clusters):
			os.makedirs("train_cls\\cluster" + str(i))
	...
	def clustering(self):
		...
		for i in range(self.max_examples):
			shutil.copy2(self.folder_path+"\\"+self.image_paths[i], "train_cls\cluster"+str(predictions[i]))
		print("\n Clustering complete! \n\n Clusters and the respective images are stored in the \"train_cls\" folder.")
...
if __name__ == "__main__":
	...
	data = "C:\\Users\\chaithu\\image-clustering-master\\train" # path of the folder that contains the images to be considered for the clustering (The folder must contain only image files)
	...
	temp = image_clustering(data, number_of_clusters, max_examples, use_imagenets, use_pca)
	temp.load_images()
